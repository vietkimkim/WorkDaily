...
def _boc_ngay(html, n):
    txt = BeautifulSoup(html, "html.parser").get_text(" ", strip=True)
    for pat in (r"ngày\s+(\d{1,2}/\d{1,2})", r"XS[A-ZĐ]{2,7}\s+(\d{1,2}/\d{1,2})"):
        ds, seen = [], set()
        for d in re.findall(pat, txt):
            if d not in seen:
...
def _gan_nam(dd_mm_moi_den_cu):
    ...
    for s in dd_mm_moi_den_cu:
        m = re.fullmatch(r"(\d{1,2})/(\d{1,2})", str(s))
        if not m:
...
def ma_tran_tin_hieu(chuoi, pool):
    ctx = dung_ctx(pool)
...
def cua_so(chuoi, pool, min_train, max_pts):
...
SIGNALS_3 = [("T1_ViTri", T1_unigram), ("T2_Ngram", T2_ngram),
             ...
             ("T7_CauTruc", T7_cautruc)]
SIG3_NAMES = [n for n, _ in SIGNALS_3]


def build_signal_matrix_3(train3):
    uni, b12, b23 = _dem_3so(train3)
    ctx = {"uni": uni, "b12": b12, "b23": b23, "lam": _lambda_3so(train3)}
...
def precompute_windows_3(draws3, min_train=30, max_pts=None):
...
def xay_lich(bat_buoc_lam_moi=False, im_lang=False):
    ...
    if os.path.exists(_LICH_FILE) and not bat_buoc_lam_moi:
        _LICH_MEM = json.load(open(_LICH_FILE))["lich"]
        return _LICH_MEM
    if not im_lang:
...
def chay_dai_v8(stt, ngay_moc, so_ky, Ks, cache, dung_master=True):
    ...
    for key, loai, vt, K, cap in cau_hinh_module(Ks):
        if loai == "3so":
            ...
            wins = cua_so(ch, pool, mt, mp)
            w, hr, h, per = toi_uu(wins, K, cap)
            Zf, _ = ma_tran_tin_hieu(ch, pool)
